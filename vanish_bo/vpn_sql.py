...
def insert_to_db(cnf_name):
	mydb = mysql.connector.connect(host=HOST,user=USERNAME,passwd=PASSWORD,database=DATABASE)
	mycursor = mydb.cursor()
	sql = "INSERT INTO vanish_tb (cnf_names, used) VALUES (%s, %s)"
	val = (cnf_name, "n")
...
def get_value_cnf(fresh_config):
	...
	input=(fresh_config)
	print(fresh_config)
	sql = "SELECT * FROM `vanish_tb` WHERE `cnf_names`= "+"'"+fresh_config+"'"+""
...
def update_to_db_as_used(cnf_name,typ0):
	...
	print("UPDATE_SQL  CONFIG  : ",end='',flush=True)
...
##########################################################################################
##########################################################################################
# fresh_config=get_fresh_config("V")
# print(fresh_config)
# get_value_cnf()
# update_to_db_as_used("ipvanish-US-Los-Angeles-lax-a56.ovpn","V")
# get_value_cnf()
